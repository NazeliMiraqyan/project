...
import { newCategory, popularCategory, favoriteCategory } from "../../redux/action"
...
const Navbar:React.FC=()=>{
    ...
    const buttonsArray = selector.reduce((acc:any, cur:any) => [
        
        ...acc.filter((obj:IItemProps) => obj.category !== cur.category), cur
      ], []);
            ...
            <ul>
                {buttonsArray.map((el:IItemProps, i:number)=>{
                    return(
                        ...
                        <div key={i}>
                            <li>
                                <button onClick = {()=> changeData(el.category, el.id)} 
                                    className = {isActive == el.id ? 'active' : '' }> {el.category} 
                                </button>
                            </li>
                        </div>
                    )
